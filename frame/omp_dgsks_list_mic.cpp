 ...
 * general stride kernel summation for Intel Xeon Phi.
 *
 *
 * Todo:
 *
...
{
  int    nthd, nu, n_list;
  double *u_local[ MIC_KS_NUM_THREAD ];
  std::deque<int> jobs[ MIC_KS_NUM_THREAD ];
  double workload[ MIC_KS_NUM_THREAD ];
  ...
  for ( int i = 0; i < MIC_KS_NUM_THREAD; i++ ) {
  ...
  // Initialize workload
  for ( int i = 0; i < MIC_KS_NUM_THREAD; i++ ) workload[ i ] = 0.0;
  ...
  for ( int i = 0; i < n_list; i++ ) {
    ...
    for ( int j = 0; j < MIC_KS_NUM_THREAD; j++ ) {
...
#pragma omp parallel for num_threads( MIC_KS_NUM_THREAD )
  for ( int i = 0; i < MIC_KS_NUM_THREAD; i++ ) {
    while ( !jobs[ i ].empty() ) {
